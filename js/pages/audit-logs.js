(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const table = DataTable.create({
      ...
      columns: [
        { key: 'userName', label: 'User', render: (r) => escapeHtml(r.userName || r.user?.name || 'System') },
        { key: 'action', label: 'Action', render: (r) => `<span class="badge badge-outline">${escapeHtml(titleCaseFromEnum(r.action))}</span>` },
        { key: 'module', label: 'Module', render: (r) => escapeHtml(titleCaseFromEnum(r.module || r.resource || '')) },
        { key: 'details', label: 'Details', render: (r) => escapeHtml((r.details || r.description || '—').toString().slice(0, 80)) },
